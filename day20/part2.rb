...
input = File.read(file_path).split("\n").each_with_index do |line, index|
    original[index] = line.to_i * 811589153
    current.push(index)
end
...
10.times do
    original.each do |key, value|
        ...
        if new_index <= -1 * current.size
            mul = new_index.abs / current.size
            new_index = ((new_index.abs%current.size) * -1) - mul
        end
        ...
        if new_index >= current.size
            mul = new_index.abs / current.size
            new_index = (new_index.abs%current.size) + mul
        end
